...
var wordsList = function(str ,subStr){
        ...
        var key = new RegExp('\\.|,|\\?|!|:|;|"', 'gui');
        var str1 = str.replace(key , "").toLowerCase().split(" ").filter((arrItem) => arrItem.indexOf(subStr) > -1);

        var res = new Set();

        str1.forEach((arrItem)=> {
               res.add(arrItem);
        });
...
var myDate =  new Date();
var getLocalDate = function(date, isSeconds, isISO){
        ...
        var isoDate = function (){
                ...
                date = date.replace("T", ", ").replace("Z", " ").replace(".", " ").split(" ").slice(0, -2).join(" ");
        ...
        if(isSeconds === true && isISO ===false ){
                return newDate + thisIs();
        }
        else if (isSeconds === true && isISO === true){
                return isoDate() + thisIs();
        }
        else if (isSeconds === false &&  isISO === false){
                return newDate;
        }
...
console.log("-----------------");
console.log(getWeekDay('2019-01-25')); // среда
console.log(getWeekDay('2019-07-16')); // вторник
console.log(getWeekDay('2019-07-27')); // суббота
console.log("-----------------");

var getLocalDay = function(date){
        var kkk = new Date(date);
        var day = kkk.getDay();
        if(day === 0){
                day = 7;  
        }
...
var getDateAgo = function(date, days){
        var kkk = new Date(date);
        kkk.setDate(kkk.getDate() - days);
        kkk = kkk.toISOString();
        kkk = kkk.replace("T", " ").replace("Z", " ").replace(".", " ").split(" ").slice(0, -3).join(" ");
        kkk = kkk.replace("-", " ").replace("-", " ").split(" ");
        [kkk[0] , kkk[3]] = [kkk[3] , kkk[0]];
        [kkk[1] , kkk[2]] = [kkk[2] , kkk[1]];
        kkk = kkk.slice(1).join(".");

        return kkk;
}

console.log(getDateAgo('2019-01-29', 30));
...
var car2 = new Car(5000, 'FX50 AWD', 'Infinite', 2020);
var car1 = new Car(4000, "Panamera", "Porsche", 2018);

console.log(car1.info()); // chevro Lacetti, 2000cc, year 2010, used
car1.used = 'new';
console.log(car1.info()); // chevro Lacetti, 2000cc, year 2019, new -- год изменен
car1.used = 'used';
console.log(car1.info()); // chevro Lacetti, 2000cc, year 2019, new -- изменения не выполняются
